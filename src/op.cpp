...
Color Material::shade(const Ray& incident, const bool isSolid) const
{
	...
	// if(isSolid){
		...
		if(isSolid) {reflectedColor = (lightQty + Ia) * color;}
		else {reflectedColor = lightQty + Ia;}
		// return reflectedColor;
	// }
	
	if(!isSolid && incident.getLevel() < 4) {
		double c = 0;
		Color kb(0.0, 0.0, 0.0); //Beers law
		...
		reflectedColor =  world->shade_ray(reflectedRay) * km;
		if(dotProduct(incident.getDirection(), incident.getNormal())
			...
			// Exiting Dielectric
			kb.R(pow(2.71828, -incident.getParameter()*8));
			kb.G(pow(2.71828, -incident.getParameter()*3));
			kb.B(pow(2.71828, -incident.getParameter()*8));
			// check TIR
		...
		return reflectedColor * R + (refractedColor * kb) *(1.0 - R)  ;
