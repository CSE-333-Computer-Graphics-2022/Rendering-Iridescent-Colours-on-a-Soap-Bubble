...
public:  
	Ray(const Vector3D& o, const Vector3D& d, int _level = 0, float _ref_idx = 1.0):
    		origin(o), direction(d), hit(false), level(_level), refractive_index(_ref_idx), t(float(1e10))
	{
		direction.normalize();	
	}
	...
	void setNormal(Vector3D _normal){normal = _normal;}
	void setLevel(int l){level = l;}
	float getEta() const {return refractive_index;}
};
#endif
